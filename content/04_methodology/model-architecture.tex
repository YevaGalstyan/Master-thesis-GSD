...
Following \textcite{vuleticFinGANForecastingClassifying2024}, the cost function combination and the corresponding $G$ with the highest $\mathrm{SR}_{w}$ on the validation data split were chosen. All combinations were further evaluated on the test split. $\mathrm{SR}_{w}$ measures directional accuracy, while option prices depend on the shape of the return distribution. Therefore, the generated returns were additionally evaluated on distributional statistics in Sec.~\ref{sec:evaluation}.
