...
The generated returns of the ForGAN baseline are reported for the first two configurations of Table~\ref{tab:configurations}, which differ only in their training schedule. The generated returns are compared to the statistical properties of the test split, from Sec.~\ref{sec:properties-ttf}.

Table~\ref{tab:baseline_training_length} reports both configurations alongside the test split. The reference values were computed from the rows of the test window matrix. With $l = 10$, the 401 returns of the test split give $n_{\mathrm{cond}} = 391$ rows.
