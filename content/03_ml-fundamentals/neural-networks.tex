...
Recurrent layers are updated sequentially, one step at a time. At each step, a recurrent layer takes the current input and a hidden state carried from the previous step to produce the output and the next hidden state
\begin{equation}
h^{(t)} = f\!\left(h^{(t-1)}, x^{(t)}; \theta\right),
\end{equation}
where $h^{(t)}$ is the hidden state at step $t$ and $x^{(t)}$ is the input at step $t$. The final hidden state $h^{(T)}$ is a fixed-length vector representation of the entire sequence, where $T$ is the length of the sequence.

The dimension of the hidden state is called the hidden size of the recurrent layer and determines the capacity of the layer. The hidden size is a hyperparameter that is set before training.
...
\todo{
   1. why a feedforward network cannot process an ordered sequence - done
   2. hidden state carried across time steps, final hidden state as a fixed-length vector representation of the sequence - done
   3. this is what 3.2.3 means by the recurrent layer processing $c$ - done
   4. vanishing gradients over long sequences, 
   5. LSTM cell
   6. state and gates, kept qualitative, cite Hochreiter and Schmidhuber (1997)
   7. brief GRU mention only, since ForGAN permits it but LSTM is used - done
}
%  4. hidden size $R$ as the width of the recurrent layer, tying
