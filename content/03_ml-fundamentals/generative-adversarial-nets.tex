...
Generative modeling is an unsupervised learning approach. A generative model takes a set of training examples and learns the probability distribution that produced them. GANs are generative models first introduced by \textcite{goodfellowGenerativeAdversarialNetworks2014}.
...
GANs are based on game theory, which studies situations where several decision-makers are present. Each participant chooses an action, and the outcome depends on the choices of all of the participants. A GAN has two decision-makers, \textbf{the generator (G)} and \textbf{the discriminator ($D$)}, typically implemented as neural networks. $G$ takes a vector of random noise as input and generates a sample as output. $D$ takes a real or generated data sample as input and outputs a probability that the input sample is real. The goal of $G$ is to fool $D$ by generating realistic-looking data samples. The goal of $D$ is to distinguish real samples from generated ones.
...
The cost function of $D$ is defined as a standard cross-entropy
\begin{equation}
J^{(D)}\bigl(\theta_d, \theta_g\bigr)
= -\frac{1}{2}\,\mathbb{E}_{\boldsymbol{x} \sim p_{\text{data}}} \log D(\boldsymbol{x})
  - \frac{1}{2}\,\mathbb{E}_{\boldsymbol{z}} \log\bigl(1 - D\bigl(G(\boldsymbol{z})\bigr)\bigr).
\end{equation}

Learning the zero-sum game minimizes the Jensen-Shannon divergence between $p_{\mathrm{data}}$ and $p_g$. Convergence holds only if both players are updated in function space. In practice, $D$ and $G$ are neural networks and updated in parameter space. $D$ minimizes a cross-entropy while $G$ maximizes the same quantity. Generated samples are rejected with high confidence at the start of training, and the gradient of $G$ vanishes at the point at which it performs worst \parencite{goodfellowNIPS2016Tutorial2017}.
\clearpage
Therefore, the cost function of $G$ is constructed by flipping the target of the cross-entropy.

\begin{equation}
J^{(G)}(\theta_d, \theta_g)
= -\frac{1}{2}\,\mathbb{E}_{p_{\boldsymbol{z}}}
  \bigl[\log[D(G(\boldsymbol{z};\theta_g);\theta_d)]\bigr].
  \label{eq:generator-cross}
\end{equation}

The cost function of $D$ and $G$ do not sum to zero anymore, hence the game is no longer zero-sum. Alternative cost functions have been introduced over time. One example is Wasserstein GAN, which replaces the Jensen–Shannon divergence between $p_{\mathrm{data}}$ and $p_g$ with the Wasserstein-1 distance \parencite{arjovskyWassersteinGAN2017}.

Figure ~\ref{fig:standard-gan} is the visualization of a standard GAN workflow adapted from \parencite{vuleticFinGANForecastingClassifying2024}. 
...
\begin{equation}
J^{(D)}\bigl(\theta_d, \theta_g\bigr)
= -\frac{1}{2}\,\mathbb{E}_{\boldsymbol{x} \sim p_{\text{data}}} \log D(\boldsymbol{x}\mid c)
  - \frac{1}{2}\,\mathbb{E}_{\boldsymbol{z}} \log\bigl(1 - D\bigl(G(\boldsymbol{z}\mid c)\bigr)\mid c\bigr).
\end{equation}

Figure ~\ref{fig:cgan} is the visualization of a cGAN workflow adapted from \parencite{vuleticFinGANForecastingClassifying2024}.
...
ForGAN adds a recurrent layer to $G$ and $D$. The layer processes $c$ as an ordered sequence and constructs a vector representation. The recurrent layer can be a long short-term memory (LSTM)\parencite{hochreiterLongShortTermMemory1997} or a gated recurrent unit (GRU)  \parencite{choLearningPhraseRepresentations2014}. The high-level architecture of the ForGAN model is shown in Fig.~\ref{fig:forgan_architecture}.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/forgan_architecture.pdf}
...
\textbf{Cost function.} Fin-GAN introduces four economics-driven terms to the cost function of $G$. 
$J^{(G)}$ can be the zero-sum, cross-entropy or Wasserstein cost function. \textcite{vuleticFinGANForecastingClassifying2024} report that the Wasserstein cost function was numerically unstable during training. Hence
$J^{(G)}$ is the cross-entropy cost function given in Eq.~\ref{eq:generator-cross}.  The four remaining terms are defined below.

\begin{equation}
L^{G}(x, \hat{x}) = J^{(G)}(x, \hat{x})
- \alpha\,\mathrm{PnL}^{*}(x, \hat{x})
...
Both are identified by a standard deviation below a threshold $\epsilon=0.0002$. Fin-GAN uses normal Xavier initialization \parencite{xavier-init}, since ForGAN suffered mode collapse in 67\% of runs under He initialization \parencite{heDelvingDeepRectifiers2015}. However, no mode collapse happened with the economic-driven, under either initializations.
...
the sum of the two $\mathrm{wPnL}^{i}_{2}$ values per day. The Sharpe ratio $\mathrm{SR}_{w}$ is then

\begin{equation}
    \label{eq:sharpe-weighted}
    \mathrm{SR}_{w} = \sqrt{252} \,
    \frac{\overline{\mathrm{wPnL}}}
         {\left( \dfrac{1}{\lfloor n/2 \rfloor}
          \sum_{i=1}^{\lfloor n/2 \rfloor}
          \left( \mathrm{wPnL}^{i} - \overline{\mathrm{wPnL}} \right)^{2}
          \right)^{1/2}} ,
    \text{ where }
    \overline{\mathrm{wPnL}} = \frac{1}{\lfloor n/2 \rfloor}
        \sum_{i=1}^{\lfloor n/2 \rfloor} \mathrm{wPnL}^{i} .
\end{equation}